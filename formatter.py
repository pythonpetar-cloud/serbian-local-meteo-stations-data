...
def format_standard_station(data):
    ...
    return (
        ...
        f"Max gust: {w['gust_ms']} m/s ({w['gust']} km/h)\n\n"

        f"📊 <b> WIND AVERAGES</b>\n"
        f"15 min: {w['avg_15']['speed_ms']} m/s "
        f"💨 gust {w['avg_15']['gust_ms']} m/s "
        f"🔺 max {w['avg_15']['max_ms']} m/s "
        f"({w['avg_15']['direction']})\n"
        f"30 min: {w['avg_30']['speed_ms']} m/s "
        f"💨 gust {w['avg_30']['gust_ms']} m/s "
        f"🔺 max {w['avg_30']['max_ms']} m/s "
        f"({w['avg_30']['direction']})\n"
        f"60 min: {w['avg_60']['speed_ms']} m/s "
        f"💨 gust {w['avg_60']['gust_ms']} m/s "
        f"🔺 max {w['avg_60']['max_ms']} m/s "
